...
export default class Monatsliste {
    ...
    _html_generieren() { // <article class="monatsliste">
        ...
        monat_jahr.textContent = `${new Date(this._jahr, this._monat - 1).toLocaleString("de-DE", { // this._monat wird nummerisch übergeben
            month: "long",                                                                          // "- 1" damit man den Monatsindex erhält
            year: "numeric"
        })}`;   
        ...
        // über eintraege_array iterieren        
        this._eintraege.forEach(eintrag => {
            eintragsliste.insertAdjacentElement("beforeend", eintrag.html());
        });

        // <ul> in article.monatsliste einsetzten
